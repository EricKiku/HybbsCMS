<template>
    ...
    <el-drawer v-model="drawerVisual" :with-header="false" direction="rtl" size="35%">
        <ZoneDrawer v-if="drawerVisual" :zone="zone"></ZoneDrawer>
    </el-drawer>
    <!-- 删除对话框 -->
    <el-dialog width="30%" class="deleteDia" v-model="deleteDiaVisual" title="删除警告">
        ...
        <div class="buttons">
            <button @click="deleteZone">确定</button>
        </div>
    </el-dialog>
    <div class="zoneData analysisData">
        ...
        <div class="options">
            <el-button type="primary" @click="getZones" :icon="Refresh">刷新</el-button>
            <div class="line"></div>
            ...
            <!-- <div class="line"></div> -->
            <el-button type="success" @click="showNewZoneDia" :icon="CirclePlus">增加分区</el-button>
        ...
        <div class="message">
            <div class="table ">
                <div class="table_head">
                    ...
                    <div class="headItem">分区Name</div>
                    <div class="headItem">分区描述</div>
                    <div class="sort headItem" @click="sort('date')">分区创建时间
                    ...
                    <div class="sort headItem" @click="sort('post')">分区帖子数
                        <img v-show="sortType == 'no' || sortKey != 'post'" src="@/assets/img/sort.png" alt="">
                        <img v-show="sortType == 'asc' && sortKey == 'post'" src="@/assets/img/sort-up.png" alt="">
                        <img v-show="sortType == 'desc' && sortKey == 'post'" src="@/assets/img/sort-down.png" alt="">
                    </div>
                    <div class="sort headItem" @click="sort('follow')">分区关注数
                        <img v-show="sortType == 'no' || sortKey != 'follow'" src="@/assets/img/sort.png" alt="">
                        <img v-show="sortType == 'asc' && sortKey == 'follow'" src="@/assets/img/sort-up.png" alt="">
                        <img v-show="sortType == 'desc' && sortKey == 'follow'" src="@/assets/img/sort-down.png" alt="">
                    </div>
                    <div class="headItem">创建者</div>
                ...
                <div class="table_data scrollbar" v-show="(!tableLoading) && tableData.length != 0">
                    ...
                    <div class="table_item" @click="showDrawer(zone)"
                        v-for="(zone) in tableData.slice((currentPage - 1) * 15, currentPage * 15)" :key="zone['z_id']"
                        :class="{ editing: editStatusing && editedzId == zone['z_id'], deleteing: deleteStatusing && deletedZId == zone['z_id'] }">
                        <div class="top">
                            <img v-show="zone['z_top'] == 1" src="@/assets/img/totop.png" alt="">
                        </div>
                        <div class="col">
                            <img class="zoneImg" @click.stop="setPriviewPictureImgPath(zone['z_icon'])"
                                :src="gStore.getimgURL() + zone['z_icon']" alt="">
                        </div>
                        <div class="col">{{ zone['z_id'] }}</div>
                        <div class="col name">
                            <span v-show="!(editStatusing && editedzId == zone['z_id'])">{{ zone['z_name'] }}</span>
                            <input class="editInput" @click.stop="" v-show="editStatusing && editedzId == zone['z_id']"
                                v-model="zName" type="text">
                        </div>
                        <div class="col introduction" :title="zone['z_introduce']">
                            <span v-show="!(editStatusing && editedzId == zone['z_id'])">{{ zone['z_introduce'] }}</span>
                            <input class="editInput" @click.stop="" v-show="editStatusing && editedzId == zone['z_id']"
                                v-model="zIntroduce" type="text">
                        </div>
                        <div class="col">{{ zone['z_createDate'] }}</div>
                        <div class="col">{{ zone['z_posts'] }}</div>
                        <div class="col">{{ zone['z_follows'] }}</div>
                        <div class="col nick">{{ zone['u_nick'] }}</div>
                        <div class="col op">
                            <button v-show="!(editStatusing && editedzId == zone['z_id'])" class="editBtn"
                                @click.stop="enterEditStatus(zone['z_id'], zone['z_name'], zone['z_introduce'])">编辑</button>
                            <button v-show="!(editStatusing && editedzId == zone['z_id'])" class="deleteBtn"
                                @click.stop="openDeleteDia(zone['z_id'])">删除</button>
                            <button v-show="editStatusing && editedzId == zone['z_id']" class="sureBtn"
                                @click.stop="update(zone['z_id'])">修改</button>
                            <button v-show="editStatusing && editedzId == zone['z_id'] && zone['z_top'] == 0"
                                class="sureBtn toTop" @click.stop="toTop(zone['z_id'])">置顶</button>
                            <button v-show="editStatusing && editedzId == zone['z_id'] && zone['z_top'] == 1"
                                class="sureBtn toTop" @click.stop="canceltoTopZone(zone['z_id'])">取消置顶</button>
                        </div>
                    </div>
...
import { getZonesReq, getMessageReq, getZonesLike, deleteZoneReq, updateZone, toTopZoneReq, canceltoTopZoneReq } from "../api/index"
...
function searchZones() {
    ...
    // 分页组件隐藏
    paginationVisual.value = false
    getZonesLike(keyWords.value).then(res => {
        if (res.data == null) {
            initTableData([])
            // 初始化分页组件
            initPagination([])
...
// 右侧组件
let selectedZone = ref(0)
let selectedZoneName = ref("")
// 选中组件方法
function selectZone(rowData) {
    selectedZone.value = rowData.z_id
    selectedZoneName.value = rowData.z_name

    // 发起请求
    getMessageReq(rowData.z_id).then(res => {
    })
}
...
// 发送删除请求
function deleteZone() {
    deleteZoneReq(deletedZId).then(res => {
        if (res.status == 200) {
            message(1, "删除成功")
            // 刷新表格
            getZones()
...
function sort(sortKeyValue) {
    ...
    switch (sortKey.value) {
        case 'id':
            ...
            break;
        case 'date':
            tableData.value = BubbleSort(tableData.value, sortType.value, 'z_createDate');
            break;
        case 'post':
            tableData.value = BubbleSort(tableData.value, sortType.value, 'z_posts');
            break;
        case 'follow':
            tableData.value = BubbleSort(tableData.value, sortType.value, 'z_follows');
            break;
        default:
            break;
    }
